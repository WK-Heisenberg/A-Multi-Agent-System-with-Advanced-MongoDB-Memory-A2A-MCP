...
def create_sync_mcp_tools(mcp_tools):
    ...
    for mcp_tool in mcp_tools:
        ...
        logger.info(f"✓ Successfully created sync wrapper for: {tool_name}")
...
@click.option("--port", "port", default=8001) # Default port for this agent
def main(host, port):
    ...
    logger.info(f"Retrieved {len(async_mcp_tools)} MCP tools")
    
    sync_tools = create_sync_mcp_tools(async_mcp_tools)
    logger.info(f"Created {len(sync_tools)} synchronous tool wrappers")
