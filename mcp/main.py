...
DATABASE_NAME = os.getenv("MCP_DATABASE_NAME", "agent_memory")
MEETINGS_COLLECTION = os.getenv("MCP_MEETINGS_COLLECTION", "meeting_slots")
    ...
    if db is None or mongo_client is None: # Connect if db or client is not initialized
        ...
        if mongo_client: # Close existing client if it exists but db is None (e.g. after a fork or an issue)
            try:
                ...
                logger.error(f"Error closing existing mongo_client: {e}")
...
async def setup_initial_data_if_needed():
    if db is None:
        ...
        logger.warning("Database not initialized for initial data setup. Attempting connection.")
        await connect_to_mongo()
        if db is None: # Still no DB after trying to connect
            logger.error("Failed to connect to DB for initial data setup.")
            return
    ...
    if current_count == 0:
        logger.info("No existing slots found, adding initial data...")
        initial_slots_data = [
        ...
        logger.info(f"{len(initial_slots_data)} initial slots added to MongoDB.")
    else:
        logger.info(f"{current_count} slots already exist in the database. Initial data setup skipped.")
...
if __name__ == "__main__":
    ...
    logger.info("Starting FastMCP server on port 8000 using SSE transport.")
    logger.info("The MCP server tools will connect to MongoDB and setup initial data on their first use if needed.")
    logger.info("To run with FastMCP CLI (recommended): fastmcp run mcp.main:mcp --transport sse --port 8000")
