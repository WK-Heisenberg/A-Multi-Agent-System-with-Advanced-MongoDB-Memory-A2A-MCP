...
class RoutingAgent:
    ...
    async def _async_init_components(self, remote_agent_addresses: List[str]):
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            for address in remote_agent_addresses:
                ...
                except httpx.ConnectError as e: # type: ignore
                    logger.error(f"Failed to get agent card from {address}: {e}")
                except Exception as e:
                    logger.error(f"Failed to initialize connection for {address}: {e}", exc_info=True)
    ...
    def create_agent(self) -> Agent:
        return Agent(
            ...
            description=(
                "This Routing agent orchestrates the decomposition of the user asking for support agent"
            ),
            tools=[
                self.send_message,
            ],
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        ...
        return f"""
        **Role:** You are an expert Routing Delegator. Your primary function is to accurately delegate user inquiries regarding support issues to the appropriate specialized remote agents.
    ...
    async def send_message(
        ...
        logger.debug(f"Routing Agent - Sending to {agent_name}")
        logger.debug(f"Session context - sessionId: {sessionId}, contextId: {context_id}, taskId: {task_id}")
        ...
        logger.debug(f"Send response from {agent_name}: {send_response}")
        ...
        logger.debug(f"Response content from {agent_name}: {json_content}")
...
def _get_initialized_routing_agent_sync():
    ...
    async def _async_main():
        # Correctly map agent URLs to their default ports as defined in start_services.sh
        routing_agent_instance = await RoutingAgent.create(
            remote_agent_addresses=[
                os.getenv("SCHEDULING_AGENT_URL", "http://localhost:8001"),
                os.getenv("SUPPORT_AGENT_URL", "http://localhost:8002"),
            ]
        )
    ...
    except RuntimeError as e: # type: ignore
        if "asyncio.run() cannot be called from a running event loop" in str(e):
            logger.warning(f"Could not initialize RoutingAgent with asyncio.run(): {e}. "
