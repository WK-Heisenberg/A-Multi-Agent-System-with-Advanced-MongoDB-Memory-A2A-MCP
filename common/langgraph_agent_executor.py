...
class LangGraphAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context, event_queue):
        ...
        logger.debug(f"A2A Context - user_id: {user_id}, session_id: {session_id}")
        logger.debug(f"Mapped to LangGraph thread_id: {thread_id}")
        logger.debug(f"A2A task_id: {context.task_id}")
        logger.debug(f"A2A context_id: {context.context_id}")

        # Enhanced debugging for message extraction
        logger.debug(f"Context type: {type(context)}")
        logger.debug(f"Context message type: {type(context.message)}")
        logger.debug(f"Context message: {context.message}")
        
        if hasattr(context.message, 'parts'):
            logger.debug(f"Message parts: {context.message.parts}")
        ...
        logger.info(f"Executing LangGraph agent with query: '{query_text}'")
