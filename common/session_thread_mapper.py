...
class SessionThreadMapper:
    ...
    NOTE: This is an in-memory implementation. In a production environment with
    multiple replicas or where process restarts are common, this should be
    backed by a persistent, distributed key-value store like Redis to ensure
    that session-to-thread mappings are not lost.
    """
